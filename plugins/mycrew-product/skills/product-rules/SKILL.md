...
in every sub-project: `features.md` names each feature, and `features/F00N-<slug>.md` says what one is
once it has more to say than its name. Sub-projects keep no plane of their own, and the North Star
...
- **Ids are permanent.** `F` plus zero-padded three digits, running on from the highest taken; never
  reused, never renumbered. A gap is a retired entry, not a free number.
...
- **Work someone must do leaves for the board.** The file holds the picture, never orders hidden in prose.

...
kind: rule
description: "Use when work is being handed to workers or a worker has come back — the one rule set for running workers across the sub-projects: what a complete assignment contains, what is never handed over, how dependent pieces are sequenced, what a returning report is judged against, and the one verdict it earns, ACCEPTED or BACK. Reference, loaded by whoever dispatches."
user-invocable: false
---

# worker-orchestration — how workers are run

Whoever dispatches builds nothing: a **worker** subagent is your only instrument and the **brief** is
the only thing you hand it. You hold that brief at both ends — you write it, and when the work comes home you
...
- **Spawn a worker and hand it a complete assignment.** Agent tool, `subagent_type: "mycrew-developers:worker"`: the
...
- **The plane belongs to the chief and a worker never writes it.** It reads the features and pulls the
  code toward them; adding a feature, checking one off or editing a feature's file is nobody's move
  down here.
...
- **A plane gap named in the report is not a rejection.** The worker was right to stop at that edge:
  the gap goes up to the chief, and the re-dispatch waits for the plane to move. Never send a worker to
  fix the plane, and never count the gap against the delivery.

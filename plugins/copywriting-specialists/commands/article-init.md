...
A folder or file already there is left as is:
...
Copy, never move, never edit: requirements and limits into `docs/`, the template to the root,
material for writing into `resources/`. Say where each file went.

### 5. Write the root `CLAUDE.md`

Fill the template below from their answers and their words.

### 6. Start the repository

Run `git init`, unless the folder is already a repository, and write a `.gitignore` with
`.DS_Store`, `.venv/`, `__pycache__/`, `node_modules/` and `~$*`.

### 7. Then stop

No `git add`, no commit. Say what you created and what is still open — a template to add,
requirements to drop into `docs/` — and name the team that takes it from here: the copywriter
writes, the reviewer checks, the layouter builds.

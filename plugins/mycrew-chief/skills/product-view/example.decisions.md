# Decisions — <product name>

<!-- Template for docs/product/decisions.md at the product root — every settled choice about the PRODUCT and
     the reasoning that closed it, one file for the whole product, newest last. Product only: a
     choice about the machinery — a language, a store, a library — is a technical decision and lives
     in docs/design/decisions.md. The `P`-id is permanent. The heading is the decision; under it
     short `-` points only — why it won, what it was weighed against, where it stops working, what
     would overturn it. Heading and points together stay within 300 chars — what will not fit is two
     decisions. Never prose. A decision that no longer holds is marked superseded, never deleted. -->

## P001 — <what was chosen, and over what where there was a real alternative>

- <why this won>
- <what it was weighed against, and what that would have cost>
- <the known ceiling — where this stops working>
- <what would overturn it>

## P002 — <a decision serving one feature> (F004)

- <why, in the same short points>

## P003 — <a decision that no longer holds> — superseded by P007

- <its points stay exactly as they were written>

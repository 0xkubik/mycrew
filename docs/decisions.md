...
<!-- Every settled choice about the product and the reasoning that closed it, one file for the whole
     product, newest last. The `D`-id is permanent. The heading is the decision; under it short `-`
     points only — why it won, what it was weighed against, where it stops working, what would
     overturn it. Never prose. A decision that no longer holds is marked superseded, never deleted. -->

## D001 — The product plane is three global files — features, notes and decisions — not one set per sub-project
